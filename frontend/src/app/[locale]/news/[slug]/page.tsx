...
export async function generateMetadata(
    ...
    const article = await newsService.getBySlug(slug, locale);
...
export default async function ArticlePage({ params }: Props) {
    ...
    let article;
    try {
        article = await newsService.getBySlug(slug, locale);
    } catch (e) {
        notFound();
    }
    ...
    const { newsList: categoryNews } = await newsService.getAll({ category: article.category_slug, limit: 10, lang: locale });
    const { newsList: latestNews } = await newsService.getAll({ limit: 10, lang: locale });
        ...
        <Layout>
            <article className="max-w-[850px] mx-auto py-6 md:py-10 px-4 sm:px-6">
                <Link href={`/${article.category_slug}`} className="text-primary font-bold text-sm md:text-base mb-6 inline-block hover:underline uppercase tracking-widest">
                    {isBn ? (article.category_name_bn || article.category_name) : (article.category_name || article.category_name_bn)}
                </Link>

                <h1 className="text-4xl sm:text-5xl md:text-6xl font-black mb-8 leading-[1.1] tracking-tight text-gray-900 font-main">
                    {article.title}
                </h1>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-12">
                        {relatedNews.map(a => (
                            <Link key={a.id} href={`/news/${a.slug}`} className="group flex flex-col gap-4">
                                ...
                                <h4 className="font-black text-xl group-hover:text-primary transition-colors leading-tight font-main">
                                    {a.title}
                                </h4>
                            </Link>
                        ))}
